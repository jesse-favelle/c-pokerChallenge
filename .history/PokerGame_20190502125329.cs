...
public class PokerGame
{
       ...
       private bool IsFullHouse(List<Card> playerHand, Player Player)
    {
        ...
        {
            Player.HandRank = 6;
            Player.Total = query.OrderByDescending(x => x.Value).First().Value * 3 + query.
            OrderByDescending(x => x.Value).Last().Value * 2;
            return true;
        }
    ...
    public static bool isTwoPair(List<Card> playerHand, Player Player)
    {
        ...
        {
            Player.HandRank = 2;
            Player.Total = query.OrderByDescending(x => x.Value).First().Value * 2; + query.OrderByDescending(x => x.Value).First()
